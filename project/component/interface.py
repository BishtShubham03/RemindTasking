...
from mylogger import root
from config import API_ID, BACKUP_FOLDER
from chat import actions as chat_actions
...
class Interface:
    ...
    def backup(data):
        ...
        root.info('Backing up ' + str(backup_path))
        if not os.path.exists(BACKUP_FOLDER):
            root.info('Createing Backup folder')
            os.makedirs(BACKUP_FOLDER)
        try:
            with open(backup_path, 'r') as f:
                val += json.load(f)
        except FileNotFoundError:
            root.debug('No backup file available')
        except json.decoder.JSONDecodeError:
            root.debug('Not json format')
    ...
    def get_reply(self):
        ...
        if (status['code'] == 200 and
            ...
            root.info('rmsg is --> {}'.format(rmsg))
            root.info('raction is --> {}'.format(raction))
            root.info('resolvedQuery is --> {}'.format(user_message))
            root.info('current_prompt is --> {}'.format(current_prompt))
            root.info('users_last_prompt is --> {}'.format(users_last_prompt))
            try:
                if not resp['result']['actionIncomplete']:
                    root.info('ActionComplete, calling action {}'.format(raction))
                    ...
                    root.info('Action InComplete - {}'.format(raction))
                ...
                root.debug('API.AI default functions')
                reply = {'rmsg': rmsg, 'contexts': '', 'buttontext': [], 'table': []}
            return {'session_id': self._session_id, **reply}
        else:
            root.error('Error response from api.ai: ' + str(resp))
